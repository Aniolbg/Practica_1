...
Como hay algunas filas donde el valor de bytes es "-" se ha decidido convertir estos en NA para evitar problemas a la hora de calcular el valor final.
...
Hemos asumido que la estructura del tiempo es [dia:hora:minutos:segundos]
...
num_peticions <- freq_horas[hora_max]
cat(sprintf("La mayor cantidad de peticiones siendo %s es a las %s horas", num_peticions, hora_max))
```
...
```{r}
num_02 <- sum(data$protocol != "HTTP/0.2\"")
cat(sprintf("Hay %d de peticiones que NO tienen como protocolo HTTP/0.2", num_02))
```
